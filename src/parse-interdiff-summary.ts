...
import type { FileStatus, FileStatusType, RealPath } from "./types";
 ...
 */
export function parseInterdiffSummary(output: string, repositoryRoot: RealPath): FileStatus[] {
  ...
  for (const lineRaw of output.trim().split("\n")) {
    ...
    if (type === "R" || type === "C") {
      ...
      const fullPath = path.join(repositoryRoot, parseResult.toPath) as RealPath;
      fileStatuses.push({
        type,
