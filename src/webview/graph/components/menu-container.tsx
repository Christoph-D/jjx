...
export function Menu({ state, children, ...rest }: MenuProps) {
  ...
  // Submenu placement depends on the menu's final position, so measure once the shared
  // menu lifecycle has positioned it.
  const measureAfterPositioning = useCallback(() => {
    setPositions(measureSubmenus(registrations.current));
  }, []);

  useMenuBehavior(menuRef, state, closeAllMenus, measureAfterPositioning);
