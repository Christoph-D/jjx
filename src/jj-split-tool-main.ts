...
function main(argv: string[]): void {
  const ipcClient = new IPCClient("jj-split-tool");

  const requestId = process.env["VSCODE_JJ_SPLIT_REQUEST_ID"];
  if (!requestId) {
    fatal(new Error("Missing VSCODE_JJ_SPLIT_REQUEST_ID"));
  }

  if (argv.length < 2) {
    fatal(new Error("Usage: jj-split-tool-main.ts <left> <right>"));
  }
