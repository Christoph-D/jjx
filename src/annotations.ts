...
export function registerStatusBar(state: ExtensionState): void {
  const context = state.context;
  ...
  const statusBarHandleDidChangeActiveTextEditor = (editor: vscode.TextEditor | undefined) => {
    if (editor && editor.document.uri.scheme === "file") {
      ...
      const repository = state.workspaceSCM.getRepositoryFromUri(lastOpenedFileUri);
      if (repository) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("jj.gitFetch", async () => {
      if (lastOpenedFileUri) {
        ...
        try {
          await state.workspaceSCM.getRepositoryFromUri(lastOpenedFileUri)?.gitFetch();
        } catch (error) {
          const { showErrorMessage } = await import("./utils");
