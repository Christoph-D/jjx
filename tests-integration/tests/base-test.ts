...
import { getVscodePath } from "../global-setup";
...
import { TestRepo, newTestRepo } from "../test-repo";
...
export async function clickRemoteRefMenuItem(graphFrame: Frame, pill: Locator, text: string) {
  await expect(async () => {
    await pill.click({ button: "right" });
