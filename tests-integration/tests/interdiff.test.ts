...
test("two selected changes show diff and interdiff sections", async ({ graphFrame, testRepo, workbox }) => {
  ...
  await test.step("a from/to diff section shows by default and hides for single selections", async () => {
  ...
  await test.step("quick actions toggle between diff and interdiff", async () => {
    await selectBoth();
  ...
  await test.step("the interdiff toggle survives a commit but reverts on selection change", async () => {
    await selectBoth();
    ...
    // Changing the graph selection reverts to the default from/to diff. The
    // unrelated commit shifted the rows down, so this selects a different pair
    // (the unrelated change and B).
    await nodes.nth(1).click();
    await nodes.nth(2).click({ modifiers: ["Shift"] });
