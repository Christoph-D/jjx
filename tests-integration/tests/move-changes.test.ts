import { test, expect, handleEditor, waitForSCMView } from "./base-test";
...
test("move single file changes from parent to working copy", async ({ graphFrame, testRepo, workbox }) => {
  ...
  const scmView = await waitForSCMView(workbox, [], ["a.txt", "b.txt"]);

  const aFileItem = scmView.getByRole("treeitem", { name: /^a\.txt/ });
  await expect(aFileItem).toBeVisible();
  await aFileItem.hover();
...
test("move all working copy changes to parent", async ({ graphFrame, testRepo, workbox }) => {
  ...
  const scmView = await waitForSCMView(workbox, ["a.txt", "b.txt"], ["a.txt"]);
  const bFileItem = scmView.getByRole("treeitem", { name: /^b\.txt/ });
...
test("move all parent changes to working copy", async ({ graphFrame, testRepo, workbox }) => {
  ...
  const scmView = await waitForSCMView(workbox, [], ["a.txt", "b.txt"]);
  const parentItem = scmView.getByRole("treeitem", { name: /Parent Commit/ });
